...
- Render is configured with the free web service instance type.
- Render backend auto-deploy is configured to wait for GitHub checks to pass.
- The backend Docker image runs `alembic upgrade head` before starting the API.
- Render prompts only for the required production backend secrets; optional provider keys can be added later from the Render service settings.
